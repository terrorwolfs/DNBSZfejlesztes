...
# Import optional API integrations
import requests
import json
...
# User account routes
@routes_blueprint.route("/account", methods=['GET', 'POST'])
@login_required
def account():
...
# Booking routes
@routes_blueprint.route("/book_room/<int:room_id>", methods=['GET', 'POST']) 
@login_required
...
# Room routes
@routes_blueprint.route("/rooms")
def rooms():
    """Public view of all rooms, no login required"""
    rooms = Room.query.all()
    return render_template('rooms.html', title='Our Rooms', rooms=rooms)

@routes_blueprint.route("/room/<int:room_id>")
def room_detail(room_id):
    """Detailed view of a specific room"""
    room = Room.query.get_or_404(room_id)
    maps_api_key = APIKeys.maps_api_key()
    # Currency handling
    base_currency = "USD"
    selected_currency = request.args.get('currency', base_currency)
    ...
    if selected_currency != base_currency:
        ...
        if response.status_code == 200:
            data = response.json()
            rate = data['conversion_rates'].get(selected_currency, 1.0)
            
    # Convert room price to selected currency
    converted_price = round(room.price_per_night * rate, 2)        
    return render_template('room_detail.html', title=f'Room {room.room_number}', room=room,price = converted_price, selected_currency = selected_currency, maps_api_key=maps_api_key)
    
# Booking management routes
@routes_blueprint.route("/cancel_booking/<int:booking_id>", methods=['POST'])
@login_required
...
# Services routes
@routes_blueprint.route("/extra_services", methods=['GET'])
def extra_services():
...
@routes_blueprint.route("/request_service/<int:booking_id>", methods=['GET', 'POST'])
@login_required
...
@routes_blueprint.route("/admin/rooms")
@admin_required
def manage_rooms():
...
@routes_blueprint.route("/admin/room/delete/<int:room_id>", methods=['POST'])
@admin_required
...
@routes_blueprint.route("/admin/room/add", methods=['GET', 'POST'])
@admin_required
def add_room():
    """Add a new room"""
...
@routes_blueprint.route("/admin/room/edit/<int:room_id>", methods=['GET', 'POST'])
@admin_required
...
@routes_blueprint.route("/admin/room/toggle_availability/<int:room_id>", methods=['POST'])
@admin_required
...
# API routes
@routes_blueprint.route("/api/user_bookings")
@login_required
...
# Booking management routes
@routes_blueprint.route("/admin/bookings")
@admin_required
def manage_bookings():
...
@routes_blueprint.route("/admin/booking/delete/<int:booking_id>", methods=['POST'])
@admin_required
...
@routes_blueprint.route("/admin/booking/<int:booking_id>", methods=['GET', 'POST'])
@admin_required
...
@routes_blueprint.route("/admin/booking/confirm/<int:booking_id>", methods=['POST'])
@admin_required
...
@routes_blueprint.route("/admin/booking/cancel/<int:booking_id>", methods=['POST'])
@admin_required
...
@routes_blueprint.route("/admin/checkins")
@admin_required
def manage_checkins():
    """View all check-ins"""
...
@routes_blueprint.route("/admin/checkin/<int:booking_id>", methods=['GET', 'POST'])
@admin_required
...
@routes_blueprint.route("/admin/checkout/<int:checkin_id>", methods=['GET', 'POST'])
@admin_required
...
# Invoice routes
@routes_blueprint.route("/admin/invoices")
@admin_required
def manage_invoices():
    """View all invoices"""
...
@routes_blueprint.route("/admin/invoice/delete/<int:invoice_id>", methods=['POST'])
@admin_required
...
@routes_blueprint.route("/admin/invoice/create/<int:booking_id>", methods=['GET', 'POST'])
@admin_required
...
@routes_blueprint.route("/admin/invoice/<int:invoice_id>", methods=['GET'])
@admin_required
def view_invoice(invoice_id):
    """View an invoice"""
    invoice = Invoice.query.get_or_404(invoice_id)
    current_user = get_current_user()
    stripe_public_key = APIKeys.stripe_public_key()
    stripe_api_key = APIKeys.stripe_api_key()
    return render_template('invoice.html', title='View Invoice', invoice=invoice, current_user=current_user, stripe_public_key=stripe_public_key)

@routes_blueprint.route("/admin/invoice/mark_paid/<int:invoice_id>", methods=['POST'])
@admin_required
...
@routes_blueprint.route("/invoice/<string:public_id>")
...
@routes_blueprint.route("/admin/room_status")
@admin_required
...
@routes_blueprint.route('/api/create-checkout-session/<int:invoice_id>', methods=['POST'])
@login_required
    ...
    try:
        ...
        line_items.append({
            'price_data': {
                'currency': 'usd',
                'product_data': {
                    'name': f'Room {invoice.booking.room.room_number} - {invoice.booking.room.room_type}',
                    'description': f'Stay from {invoice.booking.start_date.strftime("%Y-%m-%d")} to {invoice.booking.end_date.strftime("%Y-%m-%d")}'
                },
        ...
        checkout_session = stripe.checkout.Session.create(
            ...
            mode='payment',
            success_url=request.host_url + url_for('routes.payment_success', invoice_id=invoice.id),
            cancel_url=request.host_url + url_for('routes.view_invoice', invoice_id=invoice.id),
        )
...
@routes_blueprint.route('/payment/success/<int:invoice_id>')
def payment_success(invoice_id):
    """Handle successful payment"""
    invoice = Invoice.query.get_or_404(invoice_id)
    invoice.is_paid = True
    invoice.payment_date = datetime.now()
    invoice.payment_method = 'Credit Card (Stripe)'
    invoice.paid_amount = invoice.total_amount
    db.session.commit()
